% This function allows you to pass the path generated to the pure pursuit controller 
% Then simulate the Extended Kalman Filter 
function[q_true,q_pred]=extended_kalman_filter(T,Ld,path,q_true,q_pred)
...
% % 
% Change these parameters the way its written
...
% q_true = zeros(5,:); % state vectors over N time steps 
q_true = zeros(5,1000); % state vectors over N time steps for kinematic model
u = zeros(2,1000); % input vectors over N time steps
% q_pred = zeros(3,:); %odometery values over N time steps
...
%     currentpath = pathroute(:,fix(max((min(u(2,k),Vmax)*t/spacing-1/spacing),1)):fix(min(min(u(2,k),Vmax)*t/spacing+1*Ld/spacing,length(pathroute)))).'; 
    [steer_angle, ~] = purePursuitController(q_pred(:,k),  L, Ld, path);
   
%     [gamma,error] = purePursuitController(q_pred(:,k),l,Ld,currentpath);

    u(steering_angle,k) = steer_angle; % required steering angle from PPC
    u(desired_velocity,k) = 1; %desired velocity
%    
%     q_true(:,k)
    ...
    q_true(:, k+1) = q_true_next; %updates the next true positon
%     %%
%     
    if mod(k,gpssamplerate) == 0 %if a gps singal is avaliable 
        ...
        V_innovovation = [x_measured; y__measured; theta_measured] - q_pred(:,k+1);%innovation  
...
for t = 0:DT:T
    ...
    u = [gamma 1];
    q_true_next = robot_odo(q_true(:,k), u, Umin, Umax, Qmin, Qmax, L, tau_g, tau_v);   
...
scatter(q_true(1,:), q_true(2,:),'b');
% scatter(q_pred(1,:), q_pred(2,:),'r');
